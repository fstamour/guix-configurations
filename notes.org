...
** TODO set better font and color for the virtual terminals

- see ~setfont -d~
- see =KMSCONS=
- IDK about the colors yet
...
** DONE add "clean" targets to justfile

#+begin_src shell
./system delete-generations 1m
#+end_src

** DONE add "clean-guile-cache" target to justfile
...
* Document how to add substitute
...
You don't need to add the signing key if the substitute provides
bit-for-bit reproducible builds. (or if they just mirror.)

But without it, if you build something custom on another computer on
the same lan, you won't be able to take advantage of it because it
won't be trusted.
...
In short: add ~--substitutes~ to the daemon.

1. edit =/etc/systemd/system/guix-daemon.service=
   - e.g. ~--substitute-urls='https://cuirass.genenetwork.org https://ci.guix.gnu.org https://bordeaux.guix.gnu.org'~
2. ~systemctl daemon-reload~
3. ~sudo systemctl restart guix-daemon~

See also [[*add "substitute discovery" on ubuntu][add "substitute discovery" on ubuntu]]

** Related

- [[https://www.blog.rekahsoft.ca/posts/guix-na-build-farm.html][Setup of a Simple Guix Build Farm and Substitute Server]]
- [[https://lists.gnu.org/archive/html/guix-devel/2024-11/msg00174.html][Re: New North American based Guix Substitute Server, cuirass.genenetwork]]
...
see package ~u-boot-rpi-arm64-efi-bin~, ~u-boot-rpi-arm64~,
~u-boot-rpi-4-32b~, ~u-boot-rpi-2~, etc
